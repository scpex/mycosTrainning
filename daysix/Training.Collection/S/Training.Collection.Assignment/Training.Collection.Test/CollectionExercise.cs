...
namespace Training.Collection.Test
{
	...
	public class CollectionExercise
	{
		...
		public void Exercise_Of_List()
		{
            ...
            {
                sc = std.Score;
                if (sc >= 0 && sc < 50) { std.Grade = "F"; }
                else if (sc < 60) { std.Grade = "D"; }
                else if (sc < 70) { std.Grade = "C"; }
                else if (sc < 80) { std.Grade = "B"; }
                else if (sc >= 80 && sc <= 100) { std.Grade = "A"; }
                else { /* Score Over Limited(0-100)*/}

                students.Insert(num, std);
                num++;
            }
		...
		public void Exercise_Of_IEnumberable()
		{
            ...
            IEnumerable<StudentInfo> seniorStudents = Students.Where(s => DateTime.Now.Year - s.Birthday.Year > 7 && s.Score>60);
